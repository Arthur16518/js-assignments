...
function encodeToRot13(str) {
    let decrypted = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz', encrypted = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
    let result = '';
    for (let i = 0; i < str.length; i++){
        let num = decrypted.indexOf(str[i]);
        if (num == -1)
            result += str[i];
        else
            result += encrypted[num];
    }
    return result;
}
...
function getCardId(value) {
    ...
    switch (value.slice(0, value.length-1)){
        ...
        case 'K':
            ...
            break;
        default:
            j = parseInt(value.slice(0, value.length-1)) - 1;
            break;
    }
